...
async function run() {
    ...
    const downNodeJS = download(`https://github.com/${options.org}/puerts/releases/download/Unity_v${options.version}/PuerTS_V8_${options.version}.tgz`, path.join(TEMP_PATH, 'remote'), { extract: true });
    ...
    glob.sync(path.join(TARGET_PATH, '/*')).forEach(filepath => {
        cp(filepath, path.join(TEMP_PATH, 'local', path.basename(filepath)))
    })
    glob.sync(path.join(TARGET_PATH, '/**/*.meta')).forEach(filepath => {
